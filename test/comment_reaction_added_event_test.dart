import 'package:test/test.dart';
import 'package:stream_dart/stream_dart.dart';

// tests for CommentReactionAddedEvent
void main() {
  final CommentReactionAddedEvent?
  instance = /* CommentReactionAddedEvent(...) */ null;
  // TODO add properties to the entity

  group(CommentReactionAddedEvent, () {
    // ActivityResponse activity
    test('to test the property `activity`', () async {
      // TODO
    });

    // CommentResponse comment
    test('to test the property `comment`', () async {
      // TODO
    });

    // Date/time of creation
    // num createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // Map<String, Object> custom
    test('to test the property `custom`', () async {
      // TODO
    });

    // String feedVisibility
    test('to test the property `feedVisibility`', () async {
      // TODO
    });

    // String fid
    test('to test the property `fid`', () async {
      // TODO
    });

    // FeedsReactionResponse reaction
    test('to test the property `reaction`', () async {
      // TODO
    });

    // num receivedAt
    test('to test the property `receivedAt`', () async {
      // TODO
    });

    // The type of event: \"feeds.comment.reaction.added\" in this case
    // String type (default value: 'feeds.comment.reaction.added')
    test('to test the property `type`', () async {
      // TODO
    });

    // UserResponseCommonFields user
    test('to test the property `user`', () async {
      // TODO
    });
  });
}
