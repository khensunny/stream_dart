import 'package:test/test.dart';
import 'package:stream_dart/stream_dart.dart';

// tests for ActivityRemovedFromFeedEvent
void main() {
  final ActivityRemovedFromFeedEvent?
  instance = /* ActivityRemovedFromFeedEvent(...) */ null;
  // TODO add properties to the entity

  group(ActivityRemovedFromFeedEvent, () {
    // ActivityResponse activity
    test('to test the property `activity`', () async {
      // TODO
    });

    // Date/time of creation
    // num createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // Map<String, Object> custom
    test('to test the property `custom`', () async {
      // TODO
    });

    // String feedVisibility
    test('to test the property `feedVisibility`', () async {
      // TODO
    });

    // String fid
    test('to test the property `fid`', () async {
      // TODO
    });

    // num receivedAt
    test('to test the property `receivedAt`', () async {
      // TODO
    });

    // The type of event: \"feeds.activity.removed_from_feed\" in this case
    // String type (default value: 'feeds.activity.removed_from_feed')
    test('to test the property `type`', () async {
      // TODO
    });

    // UserResponseCommonFields user
    test('to test the property `user`', () async {
      // TODO
    });
  });
}
