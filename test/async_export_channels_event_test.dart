import 'package:test/test.dart';
import 'package:stream_dart/stream_dart.dart';

// tests for AsyncExportChannelsEvent
void main() {
  final AsyncExportChannelsEvent? instance = /* AsyncExportChannelsEvent(...) */
      null;
  // TODO add properties to the entity

  group(AsyncExportChannelsEvent, () {
    // num createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // Map<String, Object> custom
    test('to test the property `custom`', () async {
      // TODO
    });

    // num finishedAt
    test('to test the property `finishedAt`', () async {
      // TODO
    });

    // num receivedAt
    test('to test the property `receivedAt`', () async {
      // TODO
    });

    // num startedAt
    test('to test the property `startedAt`', () async {
      // TODO
    });

    // String taskId
    test('to test the property `taskId`', () async {
      // TODO
    });

    // String type (default value: 'export.channels.success')
    test('to test the property `type`', () async {
      // TODO
    });

    // String url
    test('to test the property `url`', () async {
      // TODO
    });
  });
}
