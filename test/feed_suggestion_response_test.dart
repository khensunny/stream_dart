import 'package:test/test.dart';
import 'package:stream_dart/stream_dart.dart';

// tests for FeedSuggestionResponse
void main() {
  final FeedSuggestionResponse? instance = /* FeedSuggestionResponse(...) */
      null;
  // TODO add properties to the entity

  group(FeedSuggestionResponse, () {
    // Map<String, double> algorithmScores
    test('to test the property `algorithmScores`', () async {
      // TODO
    });

    // When the feed was created
    // num createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // UserResponse createdBy
    test('to test the property `createdBy`', () async {
      // TODO
    });

    // Custom data for the feed
    // Map<String, Object> custom
    test('to test the property `custom`', () async {
      // TODO
    });

    // When the feed was deleted
    // num deletedAt
    test('to test the property `deletedAt`', () async {
      // TODO
    });

    // Description of the feed
    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // Fully qualified feed ID (group_id:id)
    // String feed
    test('to test the property `feed`', () async {
      // TODO
    });

    // Tags used for filtering feeds
    // List<String> filterTags
    test('to test the property `filterTags`', () async {
      // TODO
    });

    // Number of followers of this feed
    // int followerCount
    test('to test the property `followerCount`', () async {
      // TODO
    });

    // Number of feeds this feed follows
    // int followingCount
    test('to test the property `followingCount`', () async {
      // TODO
    });

    // Group this feed belongs to
    // String groupId
    test('to test the property `groupId`', () async {
      // TODO
    });

    // Unique identifier for the feed
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // Number of members in this feed
    // int memberCount
    test('to test the property `memberCount`', () async {
      // TODO
    });

    // Name of the feed
    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // Capabilities the current user has for this feed
    // List<FeedOwnCapability> ownCapabilities
    test('to test the property `ownCapabilities`', () async {
      // TODO
    });

    // Follow relationships where the current user's feeds are following this feed
    // List<FollowResponse> ownFollows
    test('to test the property `ownFollows`', () async {
      // TODO
    });

    // FeedMemberResponse ownMembership
    test('to test the property `ownMembership`', () async {
      // TODO
    });

    // Number of pinned activities in this feed
    // int pinCount
    test('to test the property `pinCount`', () async {
      // TODO
    });

    // String reason
    test('to test the property `reason`', () async {
      // TODO
    });

    // double recommendationScore
    test('to test the property `recommendationScore`', () async {
      // TODO
    });

    // When the feed was last updated
    // num updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // Visibility setting for the feed
    // String visibility
    test('to test the property `visibility`', () async {
      // TODO
    });
  });
}
