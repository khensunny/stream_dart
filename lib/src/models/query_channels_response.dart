//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//

// ignore_for_file: unused_element
import 'package:stream_dart/src/models/channel_state_response_fields.dart';
import 'package:json_annotation/json_annotation.dart';
import 'package:equatable/src/equatable_utils.dart';

part 'query_channels_response.g.dart';

@JsonSerializable(
  checked: true,
  createToJson: true,
  disallowUnrecognizedKeys: false,
  explicitToJson: true,
)
class QueryChannelsResponse {
  /// Returns a new [QueryChannelsResponse] instance.
  QueryChannelsResponse({required this.channels, required this.duration});

  /// List of channels
  @JsonKey(name: r'channels', required: true, includeIfNull: false)
  final List<ChannelStateResponseFields> channels;

  /// Duration of the request in milliseconds
  @JsonKey(name: r'duration', required: true, includeIfNull: false)
  final String duration;

  bool operator ==(Object other) {
    return identical(this, other) ||
        other is QueryChannelsResponse &&
            runtimeType == other.runtimeType &&
            equals([channels, duration], [other.channels, other.duration]);
  }

  @override
  int get hashCode =>
      runtimeType.hashCode ^ mapPropsToHashCode([channels, duration]);

  factory QueryChannelsResponse.fromJson(Map<String, dynamic> json) =>
      _$QueryChannelsResponseFromJson(json);

  Map<String, dynamic> toJson() => _$QueryChannelsResponseToJson(this);

  @override
  String toString() {
    return toJson().toString();
  }
}
