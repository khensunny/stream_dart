//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//

// ignore_for_file: unused_element
import 'package:stream_dart/src/models/role.dart';
import 'package:json_annotation/json_annotation.dart';
import 'package:equatable/src/equatable_utils.dart';

part 'create_role_response.g.dart';

@JsonSerializable(
  checked: true,
  createToJson: true,
  disallowUnrecognizedKeys: false,
  explicitToJson: true,
)
class CreateRoleResponse {
  /// Returns a new [CreateRoleResponse] instance.
  CreateRoleResponse({required this.duration, required this.role});

  /// Duration of the request in milliseconds
  @JsonKey(name: r'duration', required: true, includeIfNull: false)
  final String duration;

  @JsonKey(name: r'role', required: true, includeIfNull: false)
  final Role role;

  bool operator ==(Object other) {
    return identical(this, other) ||
        other is CreateRoleResponse &&
            runtimeType == other.runtimeType &&
            equals([duration, role], [other.duration, other.role]);
  }

  @override
  int get hashCode =>
      runtimeType.hashCode ^ mapPropsToHashCode([duration, role]);

  factory CreateRoleResponse.fromJson(Map<String, dynamic> json) =>
      _$CreateRoleResponseFromJson(json);

  Map<String, dynamic> toJson() => _$CreateRoleResponseToJson(this);

  @override
  String toString() {
    return toJson().toString();
  }
}
